...
enum AppleNotesService {
    ...
    /// Returns `true` on success, `false` if the AppleScript failed.
    @MainActor
    static func sync(
        settings: AppSettings,
        sessionIndex: SessionIndex,
        ...
        return await runExport(accountName: accountName, folderName: folderName, title: title, html: html)
    }
    ...
    private static func runExport(accountName: String, folderName: String, title: String, html: String) async -> Bool {
        ...
        let source = """
            set noteBody to read POSIX file "\(safePath)"
            tell application "Notes"
    ...
    private static func buildHTML(
        ...
        if includeTranscript, !transcriptEntries.isEmpty {
            let startedAt = transcriptEntries[0].timestamp
            body += "<h2>Transcript</h2>\n"
            for entry in transcriptEntries {
                let ts = MarkdownMeetingWriter.formatRelativeTimestamp(entry.timestamp, relativeTo: startedAt)
