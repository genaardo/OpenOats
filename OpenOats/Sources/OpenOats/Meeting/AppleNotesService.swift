...
enum AppleNotesService {
            ...
            tell application "Notes"
                ...
                tell folder "\(safeFolder)" of targetAccount
                    ...
                    if (count of matchingNotes) > 0 then
                        ...
                        make new note with properties {name:"\(safeTitle)", body:noteBody}
                    end if
                end tell
            end tell
            """
    ...
    private static func buildHTML(
        ...
        var body = ""
        // Render a human-readable title as h2 so it matches the Notes/Transcript
        // section headers. The Apple Notes `name` property (used for sidebar
        // identification and idempotent matching) shows separately; the h2 here
        // gives users a consistent visual weight throughout the note.
        let displayTitle = title
            .replacingOccurrences(of: #"^\[OpenOats\] \d{4}-\d{2}-\d{2} ?"#, with: "", options: .regularExpression)
        body += "<h2>\(displayTitle.isEmpty ? "Meeting" : displayTitle)</h2>\n"
